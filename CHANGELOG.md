...
**Changed**
- The giraffe is **Ziggy**, replacing the placeholder "Gigi". Gender-neutral,
  two easy syllables for a 3-year-old, and deliberately unlike any word in the
  item set — the vocabulary games say "pig", "cow", "dog"… a lot, so a name that
  rhymed with one would muddle them (which is why "Pip" was ruled out).
- The mascot is referred to as **they/them** throughout.
- Mascot art filenames in `design/asset-list.md` are now `ziggy_*.png` (none of
  that art has been generated yet, so nothing had to be renamed on disk). Earlier
  changelog entries were updated too — it's the same character throughout.

## 2026-07-27 — New game: Patterns ("What comes next?")
...
- A fourth game, built on the shared `ChoiceGameScene`: a repeating strip of
  ...
  appears, and Ziggy says the pattern back ("cow, pig, cow, pig — so next is
...
- **Level 1 is a worked example**, like Odd One Out: Ziggy reads the pattern, hops
  along it, then carries the next picture over into the box themselves — showing the
...
- Home now has four buttons (slightly shorter); the grown-ups area gained a
  level row for it.
...
- **Level 1 is a pre-game example**: Ziggy asks "Watch me! Which one is
